...
List<int>? generateDetailedAttendanceReport(
  String subjectName,
  int maxAttendanceCount,
  List<(String, int)> dates,
  ...
  for (var i = 0; i < dates.length; i++) {
    sheet
        .cell(CellIndex.indexByColumnRow(
            columnIndex: 3 + i, rowIndex: dates.length + 3))
        .value = TextCellValue((i + 1).toString().padLeft(3, "0"));
  }
  ...
  for (var i = 0; i < attendance.length; i++) {
    ...
    sheet
        .cell(CellIndex.indexByColumnRow(
            columnIndex: 2, rowIndex: i + dates.length + 4))
        .value = TextCellValue(attendance[i].attendance.keys.length.toString());
    for (var j = 0; j < dates.length; j++) {
      var containsKey = attendance[i].attendance.containsKey(dates[j].$1);
      sheet
          .cell(CellIndex.indexByColumnRow(
              columnIndex: 3 + j, rowIndex: i + dates.length + 4))
          .value = TextCellValue(
        containsKey ? "✅" : "❌",
      );
      sheet
          .cell(CellIndex.indexByColumnRow(
              columnIndex: 2 + j, rowIndex: i + dates.length + 4))
          .cellStyle!
          .horizontalAlignment = HorizontalAlign.Center;

      //     .backgroundColor = ExcelColor.fromHexString(containsKey ? "#11dd33": "#dd3311");
    }
  }
